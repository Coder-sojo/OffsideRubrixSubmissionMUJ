...
IDENTIFIERS = [
    "appearance_id",
    "game_id_from_appearance",
    "player_id_from_appearance",
    "player_name",
    "date",
    "name_x",
    "name_y",
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if ID_COL in df.columns:
        appearance_parts = df[ID_COL].astype(str).str.split("_", n=1, expand=True)
        if appearance_parts.shape[1] == 2:
            df["game_id_from_appearance"] = pd.to_numeric(appearance_parts[0], errors="coerce")
            df["player_id_from_appearance"] = pd.to_numeric(appearance_parts[1], errors="coerce")
...
def add_frequency_features(train: pd.DataFrame, test: pd.DataFrame, columns: Iterable[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for col in columns:
        ...
        out_col = f"{col}_freq_count"
        train[out_col] = train[col].astype(str).fillna("Unknown").map(counts).astype(float)
        test[out_col] = test[col].astype(str).fillna("Unknown").map(counts).astype(float)
        train[f"log_{out_col}"] = np.log1p(train[out_col])
        test[f"log_{out_col}"] = np.log1p(test[out_col])
    return train, test
...
def add_oof_target_encoding(
    ...
    """Add smoothed historical scoring rates without leaking row labels into train features."""
    ...
    for col in columns:
        ...
        out_col = f"{col}_oof_goal_rate"
        count_col = f"{col}_train_count"
        train_values = train[col].astype(str).fillna("Unknown")
        test_values = test[col].astype(str).fillna("Unknown")

        encoded = pd.Series(global_rate, index=train.index, dtype=float)
        for tr_idx, va_idx in skf.split(train, y):
            fold_values = train_values.iloc[tr_idx]
            fold_y = y.iloc[tr_idx]
            stats = pd.DataFrame({"key": fold_values, "target": fold_y}).groupby("key")["target"].agg(["mean", "count"])
            smooth = (stats["mean"] * stats["count"] + global_rate * smoothing) / (stats["count"] + smoothing)
            encoded.iloc[va_idx] = train_values.iloc[va_idx].map(smooth).fillna(global_rate).astype(float)

        full_stats = pd.DataFrame({"key": train_values, "target": y}).groupby("key")["target"].agg(["mean", "count"])
        full_smooth = (full_stats["mean"] * full_stats["count"] + global_rate * smoothing) / (full_stats["count"] + smoothing)

        train[out_col] = encoded
        test[out_col] = test_values.map(full_smooth).fillna(global_rate).astype(float)
        train[count_col] = train_values.map(full_stats["count"]).fillna(0).astype(float)
        test[count_col] = test_values.map(full_stats["count"]).fillna(0).astype(float)
        train[f"log_{count_col}"] = np.log1p(train[count_col])
        test[f"log_{count_col}"] = np.log1p(test[count_col])
        added.extend([out_col, count_col, f"log_{count_col}"])
...
def main() -> None:
    ...
    y_for_history = train[TARGET].astype(int)
    history_columns = [
        "player_id_from_appearance",
        "name_y",
        "name_x",
    ...
    train_fe, test_fe = add_frequency_features(train_fe, test_fe, history_columns)
    train_fe, test_fe, history_features = add_oof_target_encoding(
        train_fe,
        test_fe,
        y_for_history,
        history_columns,
    ...
    metrics["data"] = {
        ...
        "features": features,
        "advanced_history_features": history_features,
    }
